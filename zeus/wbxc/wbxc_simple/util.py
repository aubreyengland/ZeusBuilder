...
def require_keys(d: dict, keys, allow_none=True):
    ...
    :param keys: The keys to check :attr:`obj` for. This can either be a single
                 ...
                 Then the inner keys need to be passed in as a tuple, with the
...
def encode_uuid(val):
    """Encode UUID as described by the documentation
...
def revoke_token(access_token, client_id, client_secret):
    token_request = requests.post(
        url="",
        # The actual API expects the parameter of `token`, not access token,
        # which is why this params dict has that key instead of our standard
        # `access_token` which is used everywhere else.
