...
MSTEAMS_API_ENDPOINT = "https://api.interfaces.records.teams.microsoft.com"
MSTEAMS_AUTH_ENDPOINT = "https://login.microsoftonline.com/common/oauth2/v2.0/authorize"
MSTEAMS_REFRESH_ENDPOINT = "https://login.microsoftonline.com/common/oauth2/v2.0/token"
...
MSTEAMS_OAUTH_DEFAULTS = {
    "scopes": MSTEAMS_SCOPES,
    "api_endpoint": MSTEAMS_API_ENDPOINT,
    "auth_endpoint": MSTEAMS_AUTH_ENDPOINT,
    "token_endpoint": MSTEAMS_REFRESH_ENDPOINT,
}


class MsTeamsTokenMgr(TokenMgrBase):
    """
    Manager class for MS Oauth apps.

    The Manager class provides methods to interact with the
    MS Graph API to authorize apps and request tokens. It also
    ...
    def parse_token_response(resp_data: dict) -> TokenResponse:
        ...
        if not all(
            ...
            raise TokenMgrError(f"MS Auth Request Failed: {err}")

        # No refresh_expires returned but refresh tokens are good for at least 90 days
        # based on docs:
        ...
        # )
        now = datetime.now()
        refresh_expires = (now + timedelta(days=90)).timestamp()
    ...
    def _send_request(self, token_url: str, client_id: str, client_secret: str, body: dict) -> dict:
        ...
        resp = self.session.request("POST", url=url, headers=headers, data=body, timeout=10)
        if not resp.ok:
            log.exception(
                f"MS Auth Request Failed: {url} {resp.status_code} {resp.reason} {resp.text}"
            )
            try:
                err_data = resp.json()
